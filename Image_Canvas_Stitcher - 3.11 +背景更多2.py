...
BACKGROUND_PATTERNS = [
    ...
    # 纹理图案
    "brick", "herringbone", "zigzag", "spiral_pattern",
...
class BackgroundRenderer:
    ...
    def _render_pattern(settings, width, height):
        ...
        pattern_handlers = {
            ...
            "brick": BackgroundRenderer._render_brick,
    ...
    def _render_spiral_pattern(settings, width, height):
        ...
        painter.setPen(QPen(line_color, 1))
        painter.setRenderHint(QPainter.Antialiasing)
        for cx in range(int(spacing), width, int(spacing * 2)):
            for cy in range(int(spacing), height, int(spacing * 2)):
                path = QPainterPath()
                path.moveTo(cx, cy)
                for t in range(1, 60):
                    angle = t * 0.3
    ...
    def _render_triangle_tile(settings, width, height):
        ...
        for row in range(0, int(height / h) + 2):
            for col in range(0, int(width / spacing) + 2):
    ...
    def _render_hexagon_tile(settings, width, height):
        ...
        for row in range(0, int(height / (r * 1.5)) + 2):
            for col in range(0, int(width / (r * 2)) + 2):
    ...
    def _render_circle_tile(settings, width, height):
        ...
        painter.setRenderHint(QPainter.Antialiasing)
        for row in range(0, int(height / spacing) + 2):
            for col in range(0, int(width / spacing) + 2):
                x = col * spacing + (spacing / 2 if row % 2 == 1 else 0)
                y = row * spacing * 0.866
    ...
    def _render_scale(settings, width, height):
        ...
        painter.setRenderHint(QPainter.Antialiasing)
        for row in range(0, int(height / radius) + 2):
            for col in range(0, int(width / radius) + 2):
        ...
        for y in range(0, height, spacing):
            for x in range(0, width, spacing):
                if (x // spacing + y // spacing) % 3 == 0:
                    painter.drawLine(x, y + spacing, x + spacing, y + spacing)
                elif (x // spacing + y // spacing) % 3 == 1:
                    painter.drawLine(x + spacing, y, x + spacing, y + spacing)
                else:
                    painter.drawLine(x, y, x, y + spacing)
        painter.end()
        return img
    ...
    def _render_radar(settings, width, height):
        ...
        for cx in range(int(spacing), width, int(spacing * 2)):
            for cy in range(int(spacing), height, int(spacing * 2)):
                max_r = min(spacing * 0.8, cx, cy, width - cx, height - cy)
                r = spacing * 0.2
                while r < max_r:
                    painter.drawEllipse(QPointF(cx, cy), r, r)
                    r += spacing * 0.15
    ...
    def _render_ripple(settings, width, height):
        ...
        for cx, cy in centers:
            max_r = math.sqrt((width - cx)**2 + (height - cy)**2)
            r = spacing * 0.3
            while r < max_r:
                painter.drawEllipse(QPointF(cx, cy), r, r)
                r += spacing * 0.4
    ...
    def _render_mosaic(settings, width, height):
        ...
        for y in range(0, height, spacing):
            for x in range(0, width, spacing):
                ...
                for bx in range(0, spacing, int(block_size)):
                    for by in range(0, spacing, int(block_size)):
                        if (bx + by) % 2 == 0:
                            painter.fillRect(x + offset_x + bx, y + by, block_size, block_size, block_color)
        painter.end()
        return img
