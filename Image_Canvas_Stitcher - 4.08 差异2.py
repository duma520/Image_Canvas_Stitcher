...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self):
        ...
        combo_diff_mode.addItem("彩色RGB", "rgb")
        combo_diff_mode.addItem("灰度", "gray")
        combo_diff_mode.addItem("直方图", "hist")
        combo_diff_mode.addItem("SSIM+热力图", "ssim")
        ...
        combo_diff_mode.setToolTip("比对方案：\n"
                                   "彩色RGB/灰度：标记差异区域\n"
                                   "直方图/SSIM/特征点/pHash：相似度（SSIM 附热力图）\n"
                                   "切换后自动重新比对，结果实时更新")
    ...
    def _do_compare(self, selected):
        ...
        if total_diff_regions == 0:
            self.status_label.setText("比对结果：未检测到明显差异，两张图片几乎完全相同。")
        else:
            self.status_label.setText(
                f"比对完成：检测到 {total_diff_regions} 处差异区域，已标记。\n"
                f"切换工具栏方案/阈值可实时重新比对。"
            )
    ...
    def _do_internal_diff(self, regions):
        ...
        if mode in ("hist", "ssim", "feat", "phash"):
            ...
            msg, regions_out, heat, ok = self._run_similarity(img_a, img_b, mode)
            fallback_rgb = (mode == "feat" and not ok)
            if fallback_rgb:
                # 未安装 OpenCV 时回退为 RGB 差异标记，保证有可见效果
                thr = int(self._settings.get("diff_threshold", 30))
                regions_out = self._diff_regions_between(img_a, img_b, thr, mode="rgb",
                                                         block_size=4, max_shift=4)
                msg += "\n（未安装 OpenCV，已自动改用 RGB 差异标记）"
            display_mode = self._heatmap_display_mode()
            show_markers = display_mode in ("both", "marker_only") or fallback_rgb
        ...
        regions_out = self._diff_regions_between(
        ...
        if regions_out:
            ...
            for (img_item, region_rect) in ((img1, r1), (img2, r2)):
                ...
                total += len(rects)
        if total == 0:
            self.status_label.setText("比对结果：未检测到明显差异，两处区域几乎完全相同。\n切换工具栏方案/阈值可实时更新。")
        else:
            self.status_label.setText(
                f"比对完成：检测到 {total // 2} 处差异区域，已标记（吸附到图片）。\n"
                f"切换工具栏方案/阈值可实时更新。"
            )
