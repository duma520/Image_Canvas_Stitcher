...
MARKERS_DIR = os.path.join(PROGRAM_DIR, "markers")  # 新增：标记存储目录
...
class MarkerManager:
    ...
    def __init__(self):
        ensure_dirs()
        self._overlays = {}  # item_id -> MarkerOverlay
    ...
    def get_overlay(self, item, create=True):
        ...
        if create:
            ...
            overlay.setPenColor(QColor(255, 0, 0))
    ...
    def load_markers_for_item(self, item):
        ...
        try:
            ...
            overlay = self.get_overlay(item, create=True)
            if overlay:
...
class ImageCanvasView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        # 标记绘制处理
        if self._main_window and self._main_window._marker_tool.mode != MarkerTool.MODE_OFF:
            if event.button() == Qt.LeftButton:
                item = self.itemAt(event.position().toPoint())
                if isinstance(item, MovableImageItem):
                    self._main_window._start_marker_draw(item, event.position().toPoint())
                    event.accept()
                    return
                else:
                    # 检查是否有覆盖层在点击位置
                    for it in self.scene().items(event.position().toPoint()):
                        if isinstance(it, MarkerOverlay):
                            self._main_window._start_marker_draw(it._parent_item, event.position().toPoint())
                            event.accept()
                            return
    ...
    def mouseMoveEvent(self, event):
        ...
        if (self._main_window and 
            ...
            self._main_window._continue_marker_draw(event.position().toPoint())
            event.accept()
            return
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._marker_manager = MarkerManager()  # 新增：标记管理器
        self._marker_tool = MarkerTool()  # 新增：标记绘制工具
        self._marker_dialog = None  # 新增：标记工具对话框
        self._current_draw_item = None  # 当前正在绘制的图片
        self._draw_start_pos = None  # 绘制起始位置
        self._draw_path = None  # 当前绘制路径
    ...
    def _continue_marker_draw(self, pos):
        ...
        if self._marker_tool.mode == MarkerTool.MODE_PEN:
            ...
            overlay.add_path(self._draw_path)
            # 更新路径继续绘制
            self._draw_path.lineTo(local_pos)
        elif self._marker_tool.mode == MarkerTool.MODE_LINE:
            # 直线：更新预览（先清除旧的再添加新的）
            # 由于直线是最终确定的，我们只记录终点，在结束时添加
            pass
    ...
    def _end_marker_draw(self):
        ...
        if overlay and self._draw_start_pos:
            # 获取当前鼠标位置（从view获取）
