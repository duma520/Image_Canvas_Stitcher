...
class BackgroundRenderer:
    ...
    def _render_herringbone(settings, width, height):
        ...
        h = spacing * 0.5
        # 从负值开始，确保覆盖整个画布
        for y in range(int(-spacing), int(height + spacing), spacing):
            for x in range(int(-spacing), int(width + spacing), spacing):
    ...
    def _render_spiral_pattern(settings, width, height):
        ...
        step = spacing * 2
        # 从负值开始，确保覆盖整个画布
        for cx in range(int(-step), int(width + step), int(step)):
            for cy in range(int(-step), int(height + step), int(step)):
                path = QPainterPath()
                path.moveTo(cx, cy)
