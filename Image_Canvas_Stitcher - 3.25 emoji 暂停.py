...
class EmojiItem(QGraphicsPixmapItem):
    ...
    def restore_parent(self, parent_item):
        """从保存状态恢复父项（不重新计算偏移，用于加载历史/自动保存）"""
        if parent_item:
            if self._parent_image:
                self._parent_image.remove_attached_emoji(self)
            self._parent_image = parent_item
            # 不重新计算 _offset，直接使用已设置的值
            self.setParentItem(parent_item)
            self.setPos(self._offset)
            self.setFlag(QGraphicsItem.ItemIsMovable, True)
...
class HistoryManager:
    ...
    def _render_thumbnail(self, items, bounds, output_path, max_size=400):
        ...
        # 收集所有需要绘制的项（图片及其吸附的Emoji）
        draw_items = []
        for item in sorted(items, key=lambda x: x.zValue()):
            if isinstance(item, MovableImageItem):
                ...
                # 添加吸附的Emoji
                for emoji in item.get_attached_emojis():
                    if not emoji or not emoji.scene():
                        continue
                    emoji_rect = emoji.sceneBoundingRect()
                    emoji_cx = emoji_rect.center().x() - bounds.left() + margin
                    emoji_cy = emoji_rect.center().y() - bounds.top() + margin
                    draw_items.append({
                        'type': 'emoji',
                        'item': emoji,
                        'cx': emoji_cx,
        ...
        # 按Z值排序绘制
        draw_items.sort(key=lambda x: x['z'])
        
        for d in draw_items:
            painter.save()
            painter.translate(d['cx'], d['cy'])
            ...
            if d['type'] == 'image':
                ...
                item = d['item']
                emoji_pixmap = item.pixmap()
                if not emoji_pixmap.isNull():
                    painter.drawPixmap(
                        -emoji_pixmap.width() / 2,
                        -emoji_pixmap.height() / 2,
                        emoji_pixmap
                    )
...
class MainWindow(QMainWindow):
    ...
    def add_emoji_to_image(self, emoji_char, parent_image):
        ...
        # 使用 add_attached_emoji 方法（它会调用 set_parent_image 并自动计算偏移）
