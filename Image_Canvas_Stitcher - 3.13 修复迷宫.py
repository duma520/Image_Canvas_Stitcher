...
class BackgroundRenderer:
        ...
        if total_cells > max_cells:
            ...
            for row in range(0, rows, step):
                for col in range(0, cols, step):
                    ...
                    y = row * radius * 0.8
                    # 使用椭圆代替半圆，性能更好
                    painter.drawEllipse(QPointF(x, y), radius * 0.6, radius * 0.4)
            painter.end()
        else:
            painter = QPainter(img)
            painter.setPen(QPen(scale_color, 1))
    ...
    def _render_maze(settings, width, height):
        ...
        # 确保迷宫尺寸合理
        cols = max(3, int(width / spacing) + 1)
        rows = max(3, int(height / spacing) + 1)
        ...
        while stack:
            ...
            if neighbors:
                ...
                if dr == 0 and dc == 1:  # 向右
                    walls[r][c][0] = False
                elif dr == 1 and dc == 0:  # 向下
                    walls[r][c][1] = False
                elif dr == 0 and dc == -1:  # 向左
                    walls[nr][nc][0] = False
                elif dr == -1 and dc == 0:  # 向上
                    walls[nr][nc][1] = False
                stack.append((nr, nc))
            else:
                stack.pop()
        ...
        # 确保入口和出口
        walls[0][0][1] = False  # 左上角出口（下墙打通）
        walls[rows-1][cols-1][1] = False  # 右下角出口（下墙打通）
