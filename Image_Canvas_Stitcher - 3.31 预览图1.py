    ...
    def to_dict(self):
        ...
        for path in self._paths:
            ...
            for i in range(path.elementCount()):
                ...
                # 【修复】将 QPainterPath.ElementType 枚举转换为整数
                # elem.type 是枚举类型，需要通过 .value 获取数值
                # 0 = MoveToElement, 1 = LineToElement, 2 = CurveToElement, 3 = CurveToDataElement
                # 【修复】正确获取元素类型值
                try:
                    # PySide6 枚举获取整数值
                    elem_type = int(elem.type)
                except (TypeError, ValueError):
                    elem_type = 1  # 默认为 LineToElement
    ...
    def from_dict(self, data):
        ...
        for path_data in data.get("paths", []):
            ...
            for point in path_data:
                ...
                # 【修复】正确判断 MoveToElement
                # PySide6 中 QPainterPath.MoveToElement 的值是 0
                if not started or elem_type == 0:  # 0 = QPainterPath.MoveToElement
                    path.moveTo(x, y)
                    started = True
                elif elem_type == 1:  # 1 = QPainterPath.LineToElement
                    path.lineTo(x, y)
                else:
                    path.lineTo(x, y)  # 其他类型默认按直线处理
...
class MarkerManager:
    ...
    def get_overlay(self, item, create=True):
        ...
        if create:
            ...
            # 【修复】MarkerOverlay 构造函数已通过父项自动添加到场景
            # 不需要再次调用 addItem，避免重复添加
    ...
    def load_markers_for_item(self, item):
        ...
        try:
            ...
            if file_size == 0:
                if os.path.exists(marker_file):
                    os.remove(marker_file)  # 删除损坏的空文件
                return
        except OSError:
            return
        ...
        try:
            ...
            if not isinstance(data, dict):
                print(f"加载标记失败: 数据格式不正确 (期望dict，实际{type(data).__name__})")
                backup_file = marker_file + ".bak"
                try:
                    os.rename(marker_file, backup_file)
                except OSError:
...
class HistoryManager:
    ...
    def save_history(self, items, title=None):
        ...
        # 【修改】传入 state_dir 以便读取标记文件
        self._render_thumbnail(movable_items, all_rect, thumb_path, state_dir=state_dir)
        entry = {
            "id": hid,
            "title": title,
            "timestamp": ts,
    ...
    def _render_thumbnail(self, items, bounds, output_path, max_size=400, state_dir=None):
        """
        渲染缩略图，包含标记内容
        
        参数:
            items: MovableImageItem 列表
            bounds: 所有图片的包围矩形
            output_path: 输出文件路径
            max_size: 最大尺寸
            state_dir: 历史记录的 state 目录（用于读取标记文件）
        """
        ...
        for item in sorted_items:
            ...
            # 【新增】渲染标记内容到缩略图
            self._render_item_markers_to_painter(item, painter, state_dir)
    ...
    @staticmethod
    def _render_item_markers_to_painter(item, painter, state_dir=None):
        """
        将图片的标记渲染到 painter 上
        
        参数:
            item: MovableImageItem 实例
            painter: QPainter 实例（已经过平移、旋转、缩放变换）
            state_dir: 历史记录目录（可选，用于加载历史记录的标记）
        """
        iid = item.item_id()
        marker_data = None
        
        # 优先从历史记录的 state_dir 加载标记
        if state_dir:
            marker_file = os.path.join(state_dir, f"marker_{iid}.json")
            if os.path.exists(marker_file):
                try:
                    with open(marker_file, "r", encoding="utf-8") as f:
                        raw = f.read()
                    ...
                    print(f"从历史记录加载标记失败 ({iid}): {e}")
        
        # 如果历史记录中没有，尝试从当前 markers 目录加载
        if marker_data is None:
            ...
            if os.path.exists(marker_file):
                try:
                    ...
                    print(f"从标记目录加载标记失败 ({iid}): {e}")
        
        # 如果没有标记数据或数据无效，直接返回
        if marker_data is None or not isinstance(marker_data, dict):
            return
        ...
        # 获取标记颜色和笔宽
        color = QColor(marker_data.get("color", "#ff0000"))
        pen_width = marker_data.get("pen_width", 3)
        
        # 【重要】缩略图可能很小，调整笔宽确保在缩略图上可见
        # painter 已经应用了 scale 变换，所以这里需要反向调整
        transform = painter.transform()
        scale_factor = transform.m11() if transform.m11() != 0 else 1.0
        # 确保笔宽在缩略图中至少为 1 像素
        adjusted_width = max(1.0, pen_width / max(scale_factor, 0.01))
        ...
        # 渲染路径
        for path_data in marker_data.get("paths", []):
            ...
            for point in path_data:
                ...
                y = point.get("y", 0)
                elem_type = point.get("type", 0)
                if not started or elem_type == 0:  # MoveToElement
                    path.moveTo(x, y)
                    started = True
                else:
                    path.lineTo(x, y)
        ...
        # 渲染直线
        for line_data in marker_data.get("lines", []):
            if len(line_data) >= 2:
                start = QPointF(line_data[0].get("x", 0), line_data[0].get("y", 0))
                end = QPointF(line_data[1].get("x", 0), line_data[1].get("y", 0))
                # 【防御】跳过长度为零的线
        ...
        # 渲染矩形
        for rect_data in marker_data.get("rects", []):
            x = rect_data.get("x", 0)
            y = rect_data.get("y", 0)
            w = rect_data.get("w", 0)
            h = rect_data.get("h", 0)
        ...
        # 渲染椭圆
        for ellipse_data in marker_data.get("ellipses", []):
            x = ellipse_data.get("x", 0)
            y = ellipse_data.get("y", 0)
            w = ellipse_data.get("w", 0)
            h = ellipse_data.get("h", 0)
        ...
        # 渲染箭头
        for arrow_data in marker_data.get("arrows", []):
            if len(arrow_data) >= 2:
                start = QPointF(arrow_data[0].get("x", 0), arrow_data[0].get("y", 0))
                end = QPointF(arrow_data[1].get("x", 0), arrow_data[1].get("y", 0))
                # 【防御】跳过长度为零的箭头
                ...
                angle = math.atan2(end.y() - start.y(), end.x() - start.x())
                # 箭头大小适应缩略图
                arrow_length = (end - start).manhattanLength()
                arrow_size = min(max(5.0, arrow_length * 0.3), 30.0 / max(scale_factor, 0.01))
                p1 = QPointF(
                    end.x() - arrow_size * math.cos(angle - math.pi/6),
                    end.y() - arrow_size * math.sin(angle - math.pi/6)
                )
        ...
        # 渲染文字
        for text_data in marker_data.get("texts", []):
            pos = QPointF(text_data.get("x", 0), text_data.get("y", 0))
            text = text_data.get("text", "")
            if text:
                # 字体大小适应缩略图
                font_size = max(6, min(pen_width + 8, 48 / max(scale_factor, 0.01)))
...
class MainWindow(QMainWindow):
    ...
    def _on_item_added(self, item):
        if isinstance(item, MovableImageItem):
            ...
            item.setData(0, z)
            self.view.mark_background_dirty()
            # 延迟加载标记，确保 item 在场景中完全初始化
            # 这里不立即加载，由 _restore_all_markers 统一处理
    ...
    def _end_marker_draw(self):
        ...
        if overlay and item and self._draw_start_pos is not None:
            ...
            if self._marker_tool.mode == MarkerTool.MODE_PEN:
                ...
                # 路径已经在 _continue_marker_draw 中实时更新，不需要额外操作
    ...
    def _restore_all_markers(self):
        ...
        # 更新状态栏信息
        if total_items > 0:
            msg = f"已自动恢复上次画面，共 {total_items} 张图片"
            if restored_count > 0:
