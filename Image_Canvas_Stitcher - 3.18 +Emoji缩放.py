...
class EmojiItem(QGraphicsPixmapItem):
    ...
    def __init__(self, emoji_char, parent_item=None, font_size=48, item_id=None):
        ...
        # 如果有父图片，建立绑定
        if parent_item:
            self.set_parent_image(parent_item)
    ...
    def set_parent_image(self, parent_item):
        """设置父图片并建立绑定"""
        if parent_item:
            # 如果之前有父图片，先分离
    ...
    def update_position_from_parent(self):
        ...
        if self._parent_image:
            ...
            self.setPos(parent_pos + self._offset)
...
class EmojiPickerDialog(QDialog):
    ...
    def _filter_emojis(self, text):
        ...
        for idx, btn in enumerate(self.emoji_buttons):
            emoji = btn.text()
            if not text:
                btn.setVisible(True)
            else:
                # 简单匹配：检查Emoji是否包含搜索关键词
                # 注意：很多Emoji在Python中无法直接按字符匹配，这里简单处理
    ...
    def _select_emoji(self, emoji_char):
        ...
        for btn in self.emoji_buttons:
            if btn.text() == emoji_char:
                btn.setStyleSheet("background-color: #2a6ea5; border: 2px solid #4a8ec5;")
            else:
                btn.setStyleSheet("")
...
class MovableImageItem(QGraphicsPixmapItem):
    ...
    def add_attached_emoji(self, emoji_item):
        ...
        if emoji_item not in self._attached_emojis:
            ...
            emoji_item.set_parent_image(self)
